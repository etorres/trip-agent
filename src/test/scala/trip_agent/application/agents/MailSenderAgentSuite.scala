...
import trip_agent.application.agents.tools.ChatModelProvider
...
object MailSenderAgentSuite extends SimpleIOSuite:
  test("should send an email with trip findings"):
    for
...
//      _ <- ignore("not on sbt").unlessA(onSbt)
      ...
      testee = MailSenderAgent.impl(
        chatModel = chatModel,
      )
      obtained <- testee.sendEmail(
        accommodations = List(
          Accommodation(
            id = 123,
            name = "Name",
        ...
        requestId = tsid.toString,
      )
    yield expect.eql("noop@example.com", obtained)
